...
@interface SJTMonthPicker () <UITableViewDelegate, UITableViewDataSource>{
    UILabel *yearLabel;
}
...
- (SJTMonthPickerCell *)cellForMonth:(SJTMonth *)month {
    ...
    return (SJTMonthPickerCell *)[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:dayIndex inSection:0]];
}
...
- (void)setStartDate:(NSDate *)startDate endDate:(NSDate *)endDate {
    ...
    for (int i = kDefaultInitialInactiveMonths; i >= 1; i--) {
        ...
        SJTMonth *newMonth = [[SJTMonth alloc] init];
        newMonth.name = [dateNameFormatter stringFromDate:beforeDate];
        newMonth.date = beforeDate;
    ...
    for (NSDate *date = startDate; [date compare:endDate] <= 0; date = [date afterMonthDate]) {
        ...
        SJTMonth *newMonth = [[SJTMonth alloc] init];
        newMonth.name = [dateNameFormatter stringFromDate:date];
        newMonth.date = date;
    ...
    for (int i = 1; i <= kDefaultInitialInactiveMonths; i++) {
        ...
        SJTMonth *newMonth = [[SJTMonth alloc] init];
        newMonth.name = [dateNameFormatter stringFromDate:afterDate];
        newMonth.date = afterDate;
    ...
    if (tapGesture.state == UIGestureRecognizerStateEnded) {
        ...
        if (NSRangeContainsRow(self.activeMonths, indexPath.row - kDefaultInitialInactiveMonths + 1)) {
            if (indexPath.row != self.currentIndex.row) {
                ...
                _currentDate = [(SJTMonth *)self.tableMonthsData[indexPath.row] date];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (SJTMonthPickerCell *cell in self.tableView.visibleCells) {
        @autoreleasepool {
...
- (void)scrollViewDidFinishScrolling:(UIScrollView *)scrollView {
    ...
    if (centerIndexPath.row != self.currentIndex.row) {
        ...
        _currentDate = [(SJTMonth *)self.tableMonthsData[centerIndexPath.row] date];
        NSString *str = [[_currentDate getYearAndMonth] substringToIndex:4];
        yearLabel.text = [NSString stringWithFormat:@"%@年", str];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SJTMonthPickerCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseIdentifier];
    
    if (!cell) {
        cell = [[SJTMonthPickerCell alloc] initWithSize:self.monthCellSize reuseIdentifier:reuseIdentifier];
    }
    
    SJTMonth *month = self.tableMonthsData[indexPath.row];
